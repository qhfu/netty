...
/**
 * {@link io.netty.channel.EventLoop} which uses epoll under the covers. Only works on linux!
 */
final class EpollEventLoop extends SingleThreadEventLoop {
    ...
    /**
     * The flags of the given channel was modified so update the registration
     */
        ...
        for (int i = 0; i < ready; i ++) {
            ...
            if (id == 0) {
                ...
                if (ch != null) {
                    ...
                    if (write) {
                        // force flush of data as the channel is writable again
